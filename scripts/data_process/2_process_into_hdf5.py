...
from paprle.utils.data_process_utils import get_robot_joint_states, setup_fk_model, get_ee_poses, get_command_to_qpos
...
parser.add_argument('--ep-start-idx', type=int, default=0, help='Start index for processing episodes')
...
episode_list = sorted(glob.glob(glob.escape(args.data_dir) + '*'))#[29:]
print(f"Episode nums: {len(episode_list)}")

fk_models = None
ep_start_idx = args.ep_start_idx
for idx, episode_dir in enumerate(tqdm(episode_list)):
