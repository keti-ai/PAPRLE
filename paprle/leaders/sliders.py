...
class Sliders:
    def __init__(self, robot, leader_config, env_config, render_mode='', verbose=False, *args, **kwargs):
    ...
    def __render(self):
        ...
        if 'trimesh' in self.robot.robot_config.viewer_args:
            viewer_args = self.robot.robot_config.viewer_args.trimesh
            pq = viewer_args.pq
            resolution = getattr(viewer_args, 'resolution', resolution)
            self.leader_model._scene.camera_transform = pt.transform_from_pq(pq)

        self.leader_model._scene.show(
            callback=callback,
            caption=f"[Leader] Sliders for {self.robot.name}",
            flags={'grid': True},
