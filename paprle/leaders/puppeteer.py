...
class Puppeteer:
    def __init__(self, follower_robot, leader_config, env_config, render_mode='', verbose=False, *args, **kwargs):
        ...
        self.idx_pin2state = [[id, self.leader_robot.joint_names.index(name)] for id, name in
                              enumerate(self.pin_model_joint_names) if name in self.leader_robot.joint_names]
        ...
        self.follower_min_vals, self.follower_max_vals = [], []
        self.exclude_hand_joints = []
        for leader_limb_name, leader_hand_joint_idxs in self.leader_robot.ctrl_hand_joint_idx_mapping.items():
            follower_limb_name = self.leader_config.limb_mapping[leader_limb_name]
            follower_hand_joint_idxs = self.follower_robot.ctrl_hand_joint_idx_mapping[follower_limb_name]
            for idd, hand_id in enumerate(leader_hand_joint_idxs):
                if len(follower_hand_joint_idxs) > 0:
                    fidd = idd if len(follower_hand_joint_idxs) >= idd + 1 else 0
                    follower_vals = self.follower_robot.robot_config.hand_limits[follower_limb_name][fidd]
                    follower_min_val, follower_max_val = follower_vals[0], follower_vals[1]
                else:
                    follower_min_val, follower_max_val = 0.0, 1.0
                gripper_min_val, gripper_max_val = self.leader_config.hand_limits[leader_limb_name][idd]
                scale = (follower_max_val - follower_min_val ) / (gripper_max_val - gripper_min_val)
                offset = follower_min_val  - gripper_min_val * scale
                self.follower_min_vals.append(follower_min_val)
                self.follower_max_vals.append(follower_max_val)
                self.hand_offsets.append(offset)
                self.hand_scales.append(scale)
            self.hand_ids.extend(leader_hand_joint_idxs)
            if len(follower_hand_joint_idxs) == 0:
                self.exclude_hand_joints.extend(leader_hand_joint_idxs)
        self.follower_min_vals = np.array(self.follower_min_vals)
    ...
    def __render_trimesh(self):
        ...
        idx_mapping = []
        for joint_name in self.leader_model.joint_names:
            if joint_name in self.leader_robot.joint_names:
                idx_mapping.append(self.leader_robot.joint_names.index(joint_name))
            else:
                idx_mapping.append(-1)

        def callback(scene,  **kwargs ):
            self.leader_model.update_cfg(self.last_qpos[idx_mapping])
    ...
    def launch_init(self, init_env_qpos):
        ...
        while not ((check_if_hands_thresh < gripper_th).all()):
            check_if_hands_thresh = (self.leader_states[self.hand_ids] - self.follower_min_vals) / (self.follower_max_vals - self.follower_min_vals)
            print("Curr gripper values:", self.leader_states[self.hand_ids], check_if_hands_thresh)
    ...
    def get_status(self):
        current_qpos = self.leader_states
        ...
        if self.output_type == 'delta_eef_pose':
            ...
            for limb_id, (Rt, init_Rt) in enumerate(zip(new_eef_poses, self.init_eef_poses)):
                ...
                hand_poses[follower_limb_name] = current_qpos[self.hand_mapping[leader_limb_name]]
            with self.command_lock:
                self.last_command = (local_poses, hand_poses)
            return (local_poses, hand_poses)
        else:
            with self.command_lock:
                self.last_command = current_qpos.copy()
            if self.exclude_hand_joints: return current_qpos[self.include_joints]
            else: return current_qpos
