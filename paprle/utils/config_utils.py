...
def add_info_robot_config(robot_config, verbose=True):
    ...
    if not hasattr(robot_config, 'hand_limits'):
        robot_config.hand_limits = {}
    for limb_name, hand_joint_list in robot_config.hand_joint_names.items():
        hand_limits = robot_config.hand_limits.get(limb_name, [])
        if len(hand_limits) == 0:
            hand_limits = [[0.0, 1.0]] * len(hand_joint_list)  # Default limits if not provided
        elif len(hand_limits) != len(hand_joint_list):
            raise ValueError(f"Hand limits for {limb_name} do not match the number of hand joints. Expected {len(hand_joint_list)}, got {len(hand_limits)}.")
        robot_config.hand_limits[limb_name] = hand_limits
    ...
    if verbose:
        ...
        print("- robot_config.hand_limits: ", robot_config.hand_limits)
...
def sanity_check_leader_config(leader_config, robot_config, verbose=True):
    ...
    elif 'delta_eef_pose' in leader_config.output_type and 'limb_names' in leader_config: # vision pro
        robot_name = robot_config.robot_cfg.name
        leader_config.limb_mapping = {}
        for id, limb_name in enumerate(robot_config.robot_cfg.limb_joint_names.keys()):
            if id >= len(leader_config.limb_names): break
            left_or_right = leader_config.limb_names[id]
            leader_config.limb_mapping[left_or_right] = limb_name
        if robot_name == 'g1':
