...
try:
    ...
    print("teleop_msgs not found, using dummy messages.")
...
class Feedback():
    def __init__(self, robot, leader, teleoperator, env):
        ...
        self.use_ros = False
        if leader.leader_config.type == 'puppeteer' or leader.leader_config.type == 'sim_puppeteer':
            self.use_ros = True
    ...
    def send_feedback(self):
        if self.leader.leader_config.type == 'puppeteer' or self.leader.leader_config.type == 'sim_puppeteer':
            ...
            while True:
                ...
                qpos = self.env.get_current_qpos()
                ...
                if self.leader.direct_joint_mapping:
                    ...
                    if leader_command is not None:
                        ...
                        follower_qvel = self.env.get_current_qvel()
                        follower_qeff = self.env.get_current_qeff()
                        trying_mask = (abs(follower_qvel) < 0.05)
                        leader_qpos  = leader_command
                        not_following_feedback = (follower_qpos - leader_qpos)
                        not_following_feedback[self.leader.hand_ids] = not_following_feedback[self.leader.hand_ids] / self.leader.hand_scales
                        not_following_feedback[self.leader.hand_ids] = np.clip(not_following_feedback[self.leader.hand_ids], -0.4, 0.4)
                        not_following_feedback *= trying_mask

                        not_following_feedback[self.leader.hand_ids] += -np.clip((abs(follower_qeff[self.leader.hand_ids]) - 100)* 0.001, 0.0, 0.1)
                        #print("Gripper Not following feedback:", not_following_feedback[self.leader.hand_ids])
                        if np.isnan(not_following_feedback).any():
                            print("Follower joint positions are NaN, resetting to zeros.")
                    ...
                    if self.teleoperator.target_ee_poses is not None:
                        ...
                        for limb_name in self.robot.limb_names:
                            ...
                            curr_pose: pin.SE3 = pin.updateFramePlacement(pin_solver.model, pin_solver.data, pin_solver.ee_frame_id)
                            ...
                            leader_J = pin.computeFrameJacobian(self.leader.pin_model, self.leader.pin_data, self.leader.last_pin_qpos, self.leader.eef_frame_ids[0])

                            joint_feedback = leader_J.T @ err
                            gripper_feedback[:6] = joint_feedback[:6]  # only first 6 joints are arm joints

                        follower_hand_pos = follower_qpos[self.follower_hand_inds]
                        leader_hand_pos = self.leader.last_qpos[self.leader.hand_ids]
                ...
                time.sleep(max(0, 0.01 - loop_time))
        return
