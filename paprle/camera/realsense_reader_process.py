...
def stream_camera(cam_info, color_img, depth_img, time_stamp, depth_units, shutdown=False):
    ...
    fps = getattr(cam_info, 'fps', 15)
    if cam_info.depth_resolution is not None:
    ...
    while not shutdown.value:
        ...
        try:
            ...
            while True:
                print("Camera stream interrupted, trying to reconnect...")
                try:
                    cam = rs.pipeline()
                    config = rs.config()
                    config.enable_device(str(cam_info.serial_number))
...
class RealSenseReader:
    ...
    def render(self):
        ...
        for name in self.images.keys():
            ...
            image_timestamp = self.images[name]['timestamp'].value
