...
class MujocoViz:
    def  __init__(self, robot, env=None, verbose=False, xml_string=None):
        self.robot = robot
        ...
        if len(self.sim_joint_names) == 0:
            self.sim_joint_names = self.env.rev_joint_names
    ...
    def init_viewer(self, *args, **kwargs):
        self.env.init_viewer(*args, **kwargs)
        viewer_args = getattr(self.robot.robot_config.viewer_args, 'mujoco', None)
        if viewer_args is not None:
            self.update_viewer(**viewer_args)
        return
    ...
    def set_ee_targets(self, ee_targets, ik_errs=None):
        self.ee_target_mats = ee_targets
        self.ik_errs = ik_errs

    def set_ee_target(self, ee_target, index):
        self.ee_target_mats[index] = ee_target

    def get_ik_targets(self):
        return {limb_name: pt.pq_from_transform(self.ee_target_mats[id]) for id, limb_name in enumerate(self.robot.limb_names)}
    ...
    def render(self):
        if self.ee_target_mats is not None:
            for id, limb_name in enumerate(self.robot.limb_names):
                ee_pos = self.ee_target_mats[id][:3,3]
                ee_rot = self.ee_target_mats[id][:3,:3]
